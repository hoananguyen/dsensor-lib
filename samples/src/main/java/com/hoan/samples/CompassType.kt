...
fun getDSensorTypes(defaultDisplayRotation: Int, compassType: Int?): Int {
    val sensorTypes = getCompassDirection(defaultDisplayRotation)

    return when (compassType) {
        TYPE_COMPASS -> sensorTypes
        TYPE_COMPASS_AND_DEPRECATED_ORIENTATION -> sensorTypes or TYPE_DEPRECATED_ORIENTATION
        TYPE_3D_COMPASS -> sensorTypes or TYPE_NEGATIVE_Z_AXIS_DIRECTION
        TYPE_3D_COMPASS_AND_DEPRECATED_ORIENTATION ->
            sensorTypes or TYPE_NEGATIVE_Z_AXIS_DIRECTION or TYPE_DEPRECATED_ORIENTATION
        else -> 0
    }
}

fun getCompassDirection(defaultDisplayRotation: Int): Int {
    return when (defaultDisplayRotation) {
